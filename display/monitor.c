 ...
 */
static void monitor_flush(lv_display_t * disp_drv, const lv_area_t * area, uint8_t * color_p)
{
    //lv_display_t * disp_drv = lv_display_get_driver_data(disp);
...
//    printf("x1:%d,y1:%d,x2:%d,y2:%d\n", area->x1, area->y1, area->x2, area->y2);
    ...
    for(y = area->y1; y <= area->y2 && y < disp_drv->ver_res; y++) {
        memcpy(&monitor.tft_fb[y * MONITOR_HOR_RES + area->x1], color_p, w * sizeof(color_p));
        color_p += w;
    }
#endif
 ...
 * @param color_p an array of pixel to copy to the `area` part of the screen
 */
static void monitor_flush2(lv_display_t * disp_drv, const lv_area_t * area, uint8_t * color_p)
{
    int32_t hres = disp_drv->hor_res;
    int32_t vres = disp_drv->ver_res;
